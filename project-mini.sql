...
select
	r.room_type,
    count(b.room_id)
from bookings as b join rooms as r on b.room_id = r.room_id
...
select
    ...
    r.room_type,
    b.check_in
from bookings as b join guests as g on b.guest_id = g.guest_id
						join rooms as r on b.room_id = r.room_id
order by g.guest_name, r.room_type, b.check_in;

-- Cho biết mỗi khách đã đặt phòng bao nhiêu lần
select 
	g.guest_name,
    count(b.check_in) as 'Số lần đặt'
from guests as g join bookings as b on g.guest_id = b.guest_id
...
select
	r.room_id, 
    r.room_type,
    sum(b.check_out - b.check_in) * r.price_per_day as 'Tổng doanh thu'
from bookings as b join rooms as r on b.room_id = r.room_id
...
select 
	g.guest_id, 
    g.guest_name,
    count(b.booking_id) as 'Số lần đặt phòng'
from guests as g join bookings as b on g.guest_id = b.guest_id
